...
class TenantsAttributesCheckerSuite extends AnyFunSuite with SpecHelper {
  ...
  test("sendEnvelope for expiring attributes") {
    ...
    val producerSelfcare           = institution.copy(externalId = Some(producer.id.toString))
    val consumerSelfcare           = institution.copy(externalId = Some(consumer.id.toString))
    val envelopeUUID               = UUID.randomUUID()

    val consumerEnvelope: TextMail = TextMail(
      id = envelopeUUID,
      recipients = List(new InternetAddress(consumerSelfcare.digitalAddress.get)),
    ...
    val producerEnvelope: TextMail = TextMail(
      id = envelopeUUID,
      recipients = List(new InternetAddress(producerSelfcare.digitalAddress.get)),
      subject = "Scadenza attributo in 30 giorni",
      body =
