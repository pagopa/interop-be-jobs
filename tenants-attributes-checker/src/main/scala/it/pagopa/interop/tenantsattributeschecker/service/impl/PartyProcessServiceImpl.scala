...
import it.pagopa.interop.selfcare.partyprocess.client.api.{EnumsSerializers, ProcessApi}
import it.pagopa.interop.selfcare.partyprocess.client.invoker.{ApiError, ApiInvoker, ApiKeyValue}
...
import it.pagopa.interop.tenantsattributeschecker.service.PartyProcessService

import scala.concurrent.{ExecutionContext, Future}

final class PartyProcessServiceImpl(implicit system: ActorSystem[_]) extends PartyProcessService {

  private val invoker: ApiInvoker                   = ApiInvoker(EnumsSerializers.all)(system.classicSystem)
  private val api: ProcessApi                       = ProcessApi(ApplicationConfiguration.partyProcessURL)
